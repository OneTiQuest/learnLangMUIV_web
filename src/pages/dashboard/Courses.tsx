...
function Courses() {
    ...
    return (
        <div>
            <h3 className="text-muiv text-3xl font-medium">Направления подготовки</h3>

            <div className="flex items-center mt-8">
                <input type="text" ref={inputRef} placeholder="Название направления" className="form-input block w-96 p-2 mt-2 rounded-md outline-offset-2 outline-2 outline-muiv focus:outline-dashed" />
                ...
                {actionData?.error && (<span className="bg-red-300 border-2 ml-4 box-border p-4 border-red-400 rounded-md">{actionData.error_text}</span>)}
            </div>
                        ...
                        <div className="align-middle inline-block min-w-full shadow overflow-hidden sm:rounded-lg border-b border-gray-200">
                            <table className="min-w-full">
                                ...
                                <tbody className="bg-white">
                                    {Object.entries(courses).map((cource) => (
                                        <tr key={cource[1][0]}>
                                            ...
                                            <td className="px-6 py-4 whitespace-no-wrap text-left border-b border-gray-200 text-sm leading-5 font-medium" >
                                                <span onClick={deleteHandler.bind(null, cource[1][0])} className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800 ml-6 cursor-pointer">
                                                    Удалить
                                                </span>
                                            </td>
                                        </tr>
